...
#### updated Daniel van Denderen and Daniel Ottmann July 2023
...
# creating taxonomy tables for each species (need good internet for this, otherwise load from the line below).
...
# load(file = "data/data datras/ices_spp_taxo.Rdata")
...
load("data/data datras/Names_DATRAS_Walker_match.Rdata")
...
conversions <- read.csv("data/data datras/EfficiencyTab.csv",header=T,sep=",")
...
# get LME
library(rgdal)
shape1 <- readOGR(dsn = "data/data datras/LME shapefile",layer="lme66")
coords <- list.taxa %>%
...
datalw <- read.csv('data/data datras/taxa.DATRAS.FB_filled5.csv') %>% 
...
ceph_traits <- read.delim("data/ices_cephalopoda_traits2.txt", sep = "\t", stringsAsFactors = T)
