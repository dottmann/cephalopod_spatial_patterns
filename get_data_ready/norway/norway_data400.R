...
## Script name: Norway survey data at 400m depth cutoff
##
...
load(file = "data/data_cobalt_hexa_norway.Rdata")
load(file = "data/norw_dat_hexa.Rdata")
cephalopod_traits <- read.delim('data/norway_cephalopoda_traits2.txt', sep = '\t', header = T, stringsAsFactors = F)
...
temp <- norw_dat %>%
  filter(Class != "Cephalopoda", !is.na(Class))

positive_ids <- temp %>% 
  group_by(HaulID, hexa_id, Year) %>%
  summarize(haul_wgtlencpue_q = sum(wgtlencpue_q)) %>%
  filter(!is.na(haul_wgtlencpue_q))

temp <- temp %>%
  filter(HaulID %in% positive_ids$HaulID)

nrow(subset(temp, is.na(wgtlencpue_q))) / nrow(temp)

survey_data <- norw_dat %>%
  filter(!is.na(Class), HaulID %in% positive_ids$HaulID) %>%
  rename(depth = Depth,
         spp = Species) %>%
  filter(lat > 62)

rm(list = c("temp", "positive_ids"))
...
# save(cephalopod_traits, file = "C:/Users/danot/My Drive/_postdoc/projects/cephalopods_WP1/traits data base/data/norw_ceph_traits.Rdata")
...
data_cobalt_norway <- data_cobalt %>%
  filter(hexa_id %in% unique(survey_data$hexa_id)) %>%
  # rename(lon = long) %>%
  mutate(lon = case_when(lon >180 ~ lon - 360,
                         T ~ lon))

coord <-data.frame(Longitude = data_cobalt_norway$lon, Latitude = data_cobalt_norway$lat)
coordinates(coord)<- ~ Longitude + Latitude
crs(coord) <- "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"
shape <- spTransform(shape, CRS(proj4string(coord))) # make it similar to bargrid
shape@proj4string # check coordinates reference system again
tr <- over(coord, shape)
data_cobalt_norway$eco_code <- tr[,1]
data_cobalt_norway$eco_reg  <- tr[,2]
data_cobalt_norway <- subset(data_cobalt_norway, !is.na(eco_reg))
table(data_cobalt_norway$eco_reg)
...
data_cobalt_norway400 <- data_cobalt_norway  %>%
  filter(depth <= 400)

data_cobalt_norway <- data_cobalt_norway %>%
  dplyr::select(-depth)
...
p <- ggplot() +
  ...
  geom_point(data = data_cobalt_norway, aes(x = lon, y = lat), color = "grey") +
  geom_point(data = data_cobalt_norway400, aes(x = lon, y = lat), color = "red") +
  ...
  coord_map("gilbert", xlim = c(-20, 45), ylim = c(55, 83)) +
  theme_bw() 
...
df_cobalt <- df_fish %>%
  ...
  filter(!is.na(eco_reg), eco_reg != "North and East Iceland")
...
# save(stats_df, file = "data/norway_cephalopods_stats_data_sensitivity_03q.Rdata")
# save(stats_df, file = "data/norway_cephalopods_stats_data_sensitivity_noq.Rdata")
# save(stats_df, file = "data/norway_cephalopods_stats_data_sensitivity_size_q.Rdata")
