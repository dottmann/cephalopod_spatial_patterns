...
## Script name: Combine Norway survey data with cobalt & hexagons
##
...
load(file = "data/NORBTSdec2022_Ceph.RData")
...
norw_dat_latlon <- norw_dat %>%
  rename(lat = ShootLat,
         lon = ShootLong)

# Convert coordenates into geometric points:
coords <- norw_dat_latlon %>%
...
overlap <- overlap %>%
  rename(hexa_id = row.id,
         norw_dat_latlon = col.id) %>%
  arrange(norw_dat_latlon)

# Add hexa_id to each survey entry:
hexa_id <- overlap$hexa_id
norw_dat <- cbind(norw_dat, hexa_id)
...
norw_dat <- norw_dat %>% left_join(hexa_centroids1)
...
# Outfile:
# save(norw_dat, file = "data/norw_dat_hexa.Rdata")
# save(data_cobalt, file = "data/data_cobalt_hexa_norway.Rdata")
